...
function animate () {
  ...
  if (audioData) {
    const n = 256;
    const cut = -iCtrl6 * 20;
    const d = audioData.slice(0,n).reduce((s, a) => s + a, 0)/n;
    
    if (!strobo && !pressed && (d > 64 + cut)) window.invertBack = !window.invertBack;
    if (!strobo && !pressed && (d < 64 + cut)) window.invertBack = window.invertMode;
